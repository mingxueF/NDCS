...
def compute_embedding_mat(mola, dma, molb, dmb, grids, functional,xc_code):
    ...
    functionals = ["NDCS","LDA","TF","GEA2","LC94"]
...
def compute_oneshot_densities(molA, molB, molAB, xc_code, functional, A, B, path, format, thresh, gridlvl):
    ...
    while True:
        ...
        scfemb = dft.RKS(molA)
        scfemb.xc = xc_code
        focka_ref = scfres.get_hcore()
        focka = focka_ref.copy()
        focka += vemb_mat
        scfemb.get_hcore = lambda *args: focka
        scfemb.conv_tol = 1e-12
        # Solve
        scfemb.kernel()
        dma_final = scfemb.make_rdm1()
        energy_final = scfemb.e_tot
        if count == 0:
            dma_old = dma_final.copy()
            energy_old = energy_final
        else:
            denergy = abs(energy_final - energy_old)
            ddensity = np.linalg.norm(dma_final - dma_old)
        ...
        vemb_mat = compute_embedding_mat(molA, dma_final, molB, dmb, gridsa,functional,xc_code)
        count += 1
        del scfemb

    if converged:
        if format == 'txt':
            np.savetxt(''+path+'/'+A+'_'+B+'_final_dma.txt', dma_final, delimiter='\n')
            np.savetxt(''+path+'/'+A+'_'+B+'_dmb.txt', dmb, delimiter='\n')
        else:
            print("Save the density matrix-------")
            np.save(''+path+'/'+A+'_'+B+'_final_dma.npy', dma_final)
            np.save(''+path+'/'+A+'_'+B+'_dmb.npy', dmb)

    if not converged:
        print("Energy is not converged in 15 cycles----------")
    with open(functional+'oneshot_mo_energy.json','w') as f:
